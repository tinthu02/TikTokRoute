...
EXTRA_HASHTAGS = [
    # Ẩm thực cụ thể — nhà hàng đang có 2 POI
    "diadiemanuongdalat",   # tổng hợp quán ăn ngon
    "reviewanvatdalat",     # review chi tiết địa điểm ăn
    "quanandalat",          # quán ăn đặc sản

    # Cafe — cần tên quán cụ thể hơn
    "caphengondalat",       # cafe ngon Đà Lạt
    "cafedalat2026",        # cafe trending năm gần nhất

    # Địa điểm checkin rõ tên — phục vụ route optimization
    "checkindalat",         # địa điểm check-in nổi tiếng
    "diaDiemdalat",         # điểm đến Đà Lạt
    "gocnhodalat",          # góc nhớ — hay tag địa điểm cụ thể

    # Chợ / đặc sản — đang mỏng
    "diachodalat",          # chợ đêm, chợ địa phương
]
...
RAW_CSV  = "dalat_videos_raw_extra3.csv"
POI_CSV  = "dalat_poi_extracted_extra3.csv"
